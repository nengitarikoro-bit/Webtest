...
Correcting Syntax
Fatal parsing problems were triggered by unclosed elements (`<header>`) and stray end tags (`</div>`). In order to make sure that each opening tag has a matching ending tag in the proper order (First-In-Last-Out), a thorough audit of the DOM tree was conducted.
